...
const MusicApi = {
    ...
    async getLocalTopArtists({ country, limit }) {
        const res = await http.get(
            `${baseUrl}?method=geo.gettopartists&country=${country ? country : 'spain'
    ...
    async getLocalTopTracks({ country }) {
        const res = await http.get(
            `${baseUrl}?method=geo.gettoptracks&country=${country ? country : 'spain'
    ...
    async getTopAlbumsUsingTags({ tags, limit }) {
        ...
        const getAlbumfromTag = async tag => {
            const res = await http.get(
                `${baseUrl}?method=tag.gettopalbums&tag=${tag}&api_key=${API_KEY}&format=json&limit=${limit ? limit + '' : '2'
                }`
            )
            return res
        }
